def allow_operation(operation, *args)
  authorizer = instance_double(JSONAPI::Authorization::DefaultPunditAuthorizer)
  allow(authorizer).to receive(operation).with(*args).and_return(nil)

  allow(JSONAPI::Authorization::DefaultPunditAuthorizer).to receive(:new).with(Hash).and_return(authorizer)
end
