...
RSpec.describe 'including resources alongside normal operations', type: :request do
  ...
  before do
    allow_any_instance_of(Api::V1::ArticlePolicy::Scope).to receive(:resolve).and_return(
      article_policy_scope
    )
    allow_any_instance_of(Api::V1::CommentPolicy::Scope).to receive(:resolve).and_return(
      comments_policy_scope
    )
    allow_any_instance_of(Api::V1::UserPolicy::Scope).to receive(:resolve).and_return(
      user_policy_scope
    )
  end
  ...
  describe 'GET api/v1/articles' do
    subject(:last_response) { get("api/v1/articles?include=#{include_query}") }
  ...
  describe 'GET api/v1/articles/:id' do
    ...
    subject(:last_response) { get("api/v1/articles/#{article.external_id}?include=#{include_query}") }
  ...
  describe 'PATCH api/v1/articles/:id' do
    ...
    subject(:last_response) { patch("api/v1/articles/#{article.external_id}?include=#{include_query}", json) }
  ...
  describe 'POST api/v1/articles/:id' do
    ...
    subject(:last_response) { post("api/v1/articles?include=#{include_query}", json) }
  ...
  describe 'GET api/v1/articles/:id/articles' do
    let(:article) {
      Article.create(
        external_id: "indifferent_external_id",
        author: User.create(
    ...
    subject(:last_response) { get("api/v1/articles/#{article.external_id}/articles?include=#{include_query}") }
    let!(:chained_authorizer) { allow_operation('show_related_resources', source_record: article) }

    include_examples :include_directive_tests
  end

  describe 'GET api/v1/articles/:id/article' do
    let(:article) {
      Article.create(
        external_id: "indifferent_external_id",
        author: User.create(
    ...
    subject(:last_response) { get("api/v1/articles/#{article.external_id}/article?include=#{include_query}") }
